...
class C10_CUDA_API CUDAAllocatorConfig {
 public:
  ...
  /**
   * Maximum allocation size (in MB) that will be rounded up to the next
   * power of two. Allocations larger than this will use their exact size.
   * Default is 0 (disabled), meaning all allocations are rounded.
   ...
   */
  static size_t pinned_max_round_size_mb() {
    return instance().m_pinned_max_round_size_mb;
  }

  /**
   * Maximum allocation size (in MB) that will be cached for reuse.
   * Allocations larger than this will be freed immediately when released.
   * Default is 0 (disabled), meaning all allocations are cached.
   * Set to a positive value to reduce memory fragmentation for large
   * allocations that are unlikely to be reused.
   * See https://github.com/pytorch/pytorch/issues/150517
   */
  static size_t pinned_max_cached_size_mb() {
    return instance().m_pinned_max_cached_size_mb;
  }
 ...
 private:
  ...
  size_t parsePinnedReserveSegmentSize(
      const c10::CachingAllocator::ConfigTokenizer& tokenizer,
      size_t i);
  size_t parsePinnedMaxRoundSize(
      const c10::CachingAllocator::ConfigTokenizer& tokenizer,
      size_t i);
  size_t parsePinnedMaxCachedSize(
      const c10::CachingAllocator::ConfigTokenizer& tokenizer,
      size_t i);
  ...
  // Default 0 means disabled (all allocations rounded/cached as before)
  std::atomic<size_t> m_pinned_max_round_size_mb{0};
  std::atomic<size_t> m_pinned_max_cached_size_mb{0};
